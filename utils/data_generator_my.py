...
class TrainSampler(Base):
    ...
    def __iter__(self):
        ...
        count = 0  # 计数器，用于跟踪已处理的样本数
        
        while count < self.audios_num:
            batch_meta = []
            i = 0
            while i < batch_size and count < self.audios_num:
            ...
            if batch_meta:  # 确保返回非空批次
                yield batch_meta
        
        # 完成一个epoch后重置指针，以便下一次迭代可以重新开始
        self.pointer = 0
        self.random_state.shuffle(self.indexes)
    ...
    def __len__(self):
        ...
        Returns:
          int: 一个epoch的批次数量
        """
        return (self.audios_num + self.batch_size - 1) // self.batch_size
...
class AlternateTrainSampler(Base):
    ...
    def __iter__(self):
        ...
        while count < self.sampler1.audios_num:
            self.count += 1

            if self.count % 2 == 0:
                batch_meta = []
                i = 0
                while i < batch_size and count < self.sampler1.audios_num:
        ...
        # 完成一个epoch后重置两个采样器的状态
        self.sampler1.pointer = 0
        self.sampler1.random_state.shuffle(self.sampler1.indexes)
        
        self.sampler2.queue = []
        self.sampler2.pointers_of_classes = [0] * self.sampler2.classes_num
        for k in range(self.sampler2.classes_num):
            self.sampler2.random_state.shuffle(self.sampler2.indexes_per_class[k])
    ...
    def __len__(self):
        ...
        Returns:
          int: 一个epoch的批次数量
        """
        return (self.sampler1.audios_num + self.batch_size - 1) // self.batch_size
