...
def fbank(
    ...
    spectrum = replace_rfft(strided_input)  # 自己实现的有点舍入误差，对最终结果有点小影响
    if use_power:
    ...
    mel_energies.requires_grad_(False)
    Trans_mel_energies = mel_energies.T
    Trans_mel_energies = Trans_mel_energies.to(device=device, dtype=dtype)
    Trans_mel_energies.requires_grad_(False)
    spectrum = spectrum.to(device=device, dtype=dtype)

    # sum with mel fiterbanks over the power spectrum, size (m, num_mel_bins)
    mel_energies = torch.mm(spectrum, Trans_mel_energies)
